    ...
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
          <div className="text-[11px] uppercase text-slate-500">Active drives</div>
          <div className="mt-1 text-2xl font-semibold text-white">{drives.length}</div>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
          <div className="text-[11px] uppercase text-slate-500">Students (page)</div>
          <div className="mt-1 text-2xl font-semibold text-white">{students.length}</div>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
          <div className="text-[11px] uppercase text-slate-500">Placed (page)</div>
          <div className="mt-1 text-2xl font-semibold text-emerald-400">{placed}</div>
        </div>
      </div>

      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-200">Drives</h2>
        {drives.map((d) => (
          <button
            key={d.id}
            type="button"
            onClick={() => openSummary(d.id)}
            className="w-full text-left rounded-2xl border border-slate-800 bg-slate-900/40 p-4 hover:border-slate-600 transition-colors"
          >
            <div className="flex flex-wrap justify-between gap-2">
              <div>
                <div className="text-sm font-semibold text-white">{d.title}</div>
                <div className="text-xs text-slate-400">{d.company?.name} · {d.status}</div>
              </div>
        ...
        {drives.length === 0 && <p className="text-sm text-slate-500">No drives for your department.</p>}
      </section>

      <section className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-slate-200">Students</h2>
          <input
            value={batchYear}
            ...
            <tbody>
              {students.map((s) => (
                <tr key={s.id} className="border-t border-slate-800/80">
                  <td className="px-3 py-2.5 text-slate-300">{s.rollNo}</td>
                  <td className="px-3 py-2.5 text-white">{s.name}</td>
                  <td className="px-3 py-2.5 text-slate-400">{s.batchYear}</td>
                  <td className="px-3 py-2.5 text-slate-300">{s.cgpa ?? '—'}</td>
                  <td className="px-3 py-2.5">
                    <span className={s.placementStatus === 'PLACED' ? 'text-emerald-400' : 'text-amber-400'}>
                      {s.placementStatus}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 text-slate-400">{s.applicationCount}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {summary && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 space-y-3">
            <h3 className="text-lg font-semibold text-white">{summary.title}</h3>
            <p className="text-xs text-slate-400">{summary.company?.name}</p>
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="rounded-xl border border-slate-800 p-3">
                <div className="text-[10px] text-slate-500">Cohort</div>
                <div className="text-lg font-semibold text-white">{summary.counts?.cohort}</div>
              </div>
              <div className="rounded-xl border border-slate-800 p-3">
                <div className="text-[10px] text-slate-500">Applied</div>
                <div className="text-lg font-semibold text-white">{summary.counts?.applied}</div>
              </div>
              <div className="rounded-xl border border-slate-800 p-3">
            ...
            <p className="text-[11px] text-slate-500">Package details are not shown to faculty.</p>
            <button
              type="button"
              onClick={() => setSummary(null)}
              className="w-full rounded-xl border border-slate-700 py-2 text-xs text-slate-300"
            >
              Close
            </button>
          </div>
