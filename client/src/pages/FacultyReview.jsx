      ...
      <div className="backdrop-blur-md bg-bg-card/30 border border-border-app rounded-2xl p-6">
        ...
        <div className="space-y-4">
          {requests.map((req) => (
            <div 
              key={req.id} 
              className="flex flex-col lg:flex-row justify-between items-start gap-6 p-6 rounded-2xl border border-border-app bg-bg-app/20 hover:border-border-card/80 transition-all"
            >
              <div className="flex-1 space-y-4">
                <div className="flex flex-wrap items-center gap-3">
                  <span className="text-sm font-bold text-text-main uppercase tracking-wider bg-bg-sidebar px-2.5 py-1 rounded-md border border-border-app">
                    {req.requestType}
                  </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="text-text-muted text-xs uppercase font-bold">Student Identity</p>
                    <p className="font-semibold text-text-main mt-0.5">{req.student.user.name}</p>
                    <p className="text-xs text-text-muted">
                      Roll No: {req.student.rollNo} | Sec: {req.student.section} | Batch: {req.student.batchYear}
                    </p>
                  </div>
                  <div>
                    <p className="text-text-muted text-xs uppercase font-bold">Requested Dates</p>
                    <p className="font-semibold text-text-main mt-0.5 flex items-center gap-1.5">
                ...
                <div>
                  <p className="text-text-muted text-xs uppercase font-bold mb-1">Reason Statement</p>
                  <p className="text-sm text-text-main leading-relaxed bg-bg-card/50 p-3 rounded-xl border border-border-card/50">
                    {req.reason}
                  </p>
                </div>
                ...
                {req.attachmentPath && (
                  <div>
                    <p className="text-text-muted text-xs uppercase font-bold mb-1">Supporting Document</p>
                    <a
              ...
              {/* ACTION PANEL */}
              {req.status === 'PENDING' && (
                <div className="w-full lg:w-80 shrink-0 space-y-3 bg-bg-card/30 p-4 rounded-xl border border-border-card/50">
                  <div>
                    <label className="block text-xs uppercase font-bold text-text-muted mb-1.5">
                    ...
                    <textarea
                      ...
                      className="w-full h-20 px-3 py-2 bg-bg-app border border-border-app rounded-lg text-text-main text-sm focus:outline-none focus:border-blue-500"
                    />
                  </div>
