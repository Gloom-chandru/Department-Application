  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 flex flex-col justify-between rounded-xl border border-slate-800 bg-slate-900/50 p-6 backdrop-blur-sm">
          <div>
          ...
          <div className="flex items-center gap-3 mt-4">
            <label className="text-sm font-medium text-slate-300">Semester Filter:</label>
            <select
        ...
        {/* Load Stat Card */}
        <div className="rounded-xl border border-slate-800 bg-gradient-to-br from-blue-950/20 to-slate-950 p-6 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-slate-400">Weekly Teaching Load</span>
            <BookOpen className="h-5 w-5 text-blue-400" />
          </div>
          <div className="mt-4">
            <span className="text-4xl font-extrabold text-white">{teachingLoad}</span>
            <span className="text-slate-500 text-sm ml-2">periods / week</span>
          </div>
      ...
      {slots.length > 0 ? (
        <div className="rounded-xl border border-slate-800 bg-slate-900/20 backdrop-blur-sm p-4">
                        ...
                        if (!slot) {
                          return (
                            <td key={p.id} className="p-3 border-r border-slate-800/30">
                              <div className="flex flex-col items-center justify-center h-20 rounded-lg border border-dashed border-slate-800/40 text-slate-700 text-xs select-none">
                                -
                              </div>
                            </td>
                          );
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <div className="text-[10px] font-medium text-slate-300 truncate mt-0.5">{slot.subject.name}</div>
                              <div className="flex items-center justify-between text-[10px] text-slate-400 mt-1">
                                <span className="flex items-center gap-0.5 truncate font-semibold text-slate-200">
          ...
          <div className="lg:hidden space-y-4">
            {DAYS.map((dayName, dayIndex) => {
              ...
              return (
                <div key={dayName} className="rounded-lg border border-slate-800/80 bg-slate-900/40 p-4">
                  <h3 className="font-bold text-sm text-slate-200 mb-3 border-b border-slate-800 pb-2">{dayName}</h3>
                      ...
                      return (
                        <div 
                          ...
                          className={`flex items-start gap-4 p-3 rounded-lg border transition-all duration-200 ${
                            isCurrentlyActive ? 'bg-blue-950/15 border-blue-500/50' : 'bg-slate-900/20 border-slate-800/60'
                          }`}
                        >
                          <div className="w-20 shrink-0">
                            ...
                            <span className="text-xs font-semibold text-slate-300 mt-0.5 block">{slot.startPeriod.startTime}</span>
                            <span className="text-[10px] text-slate-500 block">{slot.endPeriod.endTime}</span>
                          </div>

                          <div className="h-10 border-l border-slate-800 shrink-0"></div>

                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-1">
                              <span className="font-bold text-sm text-slate-200 block truncate">{slot.subject.code} - {slot.subject.name}</span>
                              {isCurrentlyActive && (
                                <span className="rounded bg-blue-500 px-1 py-0.5 text-[8px] font-bold uppercase text-white">LIVE</span>
                              )}
                            </div>
        ...
        <div className="flex flex-col items-center justify-center text-center p-12 border border-slate-800 rounded-xl bg-slate-900/20">
          ...
          <h3 className="text-lg font-semibold text-slate-300">No Classes Scheduled</h3>
